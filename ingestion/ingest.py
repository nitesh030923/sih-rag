...
class DocumentIngestionPipeline:
    ...
    def _transcribe_audio(self, file_path: str) -> str:
        ...
        try:
            ...
            logger.warning(f"Audio transcription not available - missing dependencies: {e}")
            logger.warning("To enable audio transcription, install: pip install docling[audio]")
            return f"[Audio transcription not available - install docling[audio] and ffmpeg]"
        except Exception as e:
            logger.error(f"Failed to transcribe {file_path} with Whisper ASR: {e}", exc_info=True)
